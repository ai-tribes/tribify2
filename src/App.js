...
• Learn about wallet management`
  ...
  useEffect(() => {
    console.log('Current subwallets:', {
      ...
      wallets: userWallets?.map(w => w.publicKey)
    });
  }, [userWallets]);

  // When WalletPage updates subwallets, it will call setUserWallets
  ...
  // Core functions
  // Remove handleConnection function since we're handling connection in LandingPage.js
  ...
  useEffect(() => {
    const autoConnect = async () => {
      try {
        // First check if we have connection info in localStorage (from LandingPage)
        const storedIsConnected = localStorage.getItem('isConnected');
        const storedPublicKey = localStorage.getItem('publicKey') || localStorage.getItem('tribify_parent_wallet');
        
        if (storedIsConnected === 'true' && storedPublicKey) {
          console.log('Found stored connection:', storedPublicKey);
          setIsConnected(true);
        ...
        // Fall back to Phantom auto-connect
...
Try asking about one of these topics or use /help to see all commands!`;
  ...
  // Initialize TribifyContext values
  const tribifyContextValue = {
    publicKeys: publicKey ? [publicKey] : [],
    setPublicKeys: (keys) => {
      console.log('Setting public keys:', keys);
    },
    userWallets: userWallets,
    setUserWallets: setUserWallets,
    subwallets: userWallets,
    setSubwallets: setUserWallets
  };

  return (
    <TribifyContext.Provider value={tribifyContextValue}>
      <div className="App">
        <div className="desktop-nav">
          ...
          <Restore onRestore={restoreNicknames} />
        </div>
